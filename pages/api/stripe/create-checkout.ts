...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const lineItems = items.map(item => {
      // Validar preço - Forçar 59.99 para todos os itens conforme regra de negócio (rollback)
    const price = 59.99;
    // const price = Number(item.price);
    // if (isNaN(price) || price <= 0) {
    //   console.warn(`Preço inválido para o item ${item.title}: ${item.price}. Usando fallback 59.99`);
    // }
    const finalPrice = 59.99;
